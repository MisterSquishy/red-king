...
}:
{
  ...
  return <>
    ...
    { otherPlayers && otherPlayers.length ? 
      otherPlayers.map((player: Player) => <p key={`${JSON.stringify(player)}`}>{ player.name }</p>) :
      <p>nobody</p>
    }
    <h3>
      what's in my hand?
    ...
    { player ?
      player.hand.cards.map(card => <p key={`${JSON.stringify(card)}`}>{card.cardName} of {card.suit}</p>) :
