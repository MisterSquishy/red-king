...
export default ({
  ...
  return <>
    { hand && hand.cards.length > 4 && <>
      ...
      />
      <button onClick={() => onDiscard(hand.cards[selectedCard])}>discard</button>
      <button onClick={() => onDiscardAndFinish(hand.cards[selectedCard])}>discard and finish</button>
    </>}
    { hand && hand.cards.length < 5 && <>
      <button onClick={() => onDraw(DrawType.DECK)}>draw from deck</button>
      <button disabled={!hasDiscard} onClick={() => onDraw(DrawType.DISCARD)}>draw from discard</button>
    </>}
  </>
}